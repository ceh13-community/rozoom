...
async function runAccessReviewsScenario(page) {
  console.log("[accessreviews] open page");
  await page.goto(clusterRoute("accessreviews"), {
    waitUntil: "domcontentloaded",
    timeout: 20_000,
  });
  await waitForPageToSettle(page, 3000);

  await page.getByRole("heading", { name: "Access Review" }).waitFor({
    state: "visible",
    timeout: 15_000,
  });
  await page.getByRole("button", { name: "Copy kubectl" }).waitFor({
    state: "visible",
    timeout: 15_000,
  });
  await page.getByRole("button", { name: "Can I?" }).click();
  await page.getByText("Result").waitFor({
    state: "visible",
    timeout: 15_000,
  });
  await page.getByText(/^(yes|no)$/).waitFor({
    state: "visible",
    timeout: 15_000,
  });
}
...
async function runPortForwardingScenario(page) {
  ...
  await page.getByRole("heading", { name: "Port Forwarding" }).waitFor({
    state: "visible",
    timeout: 15_000,
  });
  await page.getByText("Start Port Forward").waitFor({
    state: "visible",
    timeout: 15_000,
  });
  await page.getByRole("button", { name: "Refresh resources" }).waitFor({
    state: "visible",
    timeout: 15_000,
  });
  await page.getByText(/Detected: \d+|Loading resources\.\.\./).waitFor({
    state: "visible",
    timeout: 15_000,
  });
  const bodyText = await page.locator("body").innerText();
  assert.match(bodyText, /No active port-forwards\.|Running|Copy URL|Open Web|Open external|Stop/i);
}
...
async function main() {
  ...
  try {
    await runAccessReviewsScenario(page);
    await runPortForwardingScenario(page);
    await page.screenshot({
      path: "/tmp/live-qa-access-reviews-port-forwarding-smoke.png",
      fullPage: true,
    });
  } finally {
    await browser.close();
    ...
    JSON.stringify(
      {
        ...
        checkedWorkloads: ["accessreviews", "portforwarding"],
        screenshot: "/tmp/live-qa-access-reviews-port-forwarding-smoke.png",
      },
      null,
      2,
    ),
