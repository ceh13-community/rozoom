...
async function ensureClusterKubeconfigMirror() {
  ...
  if (!home) {
    throw new Error("HOME is required to mirror kubeconfig for Cluster Ops QA.");
  }
...
async function waitForPageToSettle(page, extraMs = 2200) {
  await page.waitForLoadState("domcontentloaded");
  await sleep(extraMs);
}

async function runDeprecationScanScenario(page) {
  console.log("[deprecationscan] open page");
  await page.goto(clusterRoute("deprecationscan"), {
    waitUntil: "domcontentloaded",
    timeout: 20_000,
  });
  await waitForPageToSettle(page, 3500);

  await page.getByRole("heading", { name: "Deprecated API Exposure" }).waitFor({
    state: "visible",
    timeout: 15_000,
  });
  ...
  assert.match(
    bodyText,
    /Scan unavailable|Need configuration|Last scan|Deprecation scan status|Source health/i,
  );
}

async function runVersionAuditScenario(page) {
  console.log("[versionaudit] open page");
  await page.goto(clusterRoute("versionaudit"), {
    waitUntil: "domcontentloaded",
    timeout: 20_000,
  });
  await waitForPageToSettle(page, 4500);

  await page.getByRole("heading", { name: "Cluster Version & Helm Audit" }).waitFor({
    state: "visible",
    timeout: 15_000,
  });
  await page.getByRole("button", { name: "Refresh status" }).waitFor({
  ...
  await page.getByText("Installed Helm charts").waitFor({
    state: "visible",
    timeout: 15_000,
  });
  const bodyText = await page.locator("body").innerText();
  assert.match(bodyText, /Audit unavailable|Installed Helm charts|Cluster version/i);
}

async function runBackupAuditScenario(page) {
  console.log("[backupaudit] open page");
  await page.goto(clusterRoute("backupaudit"), {
    waitUntil: "domcontentloaded",
    timeout: 20_000,
  });
  await waitForPageToSettle(page, 4500);

  await page.getByRole("heading", { name: "Cluster Backup Status" }).waitFor({
    state: "visible",
    timeout: 15_000,
  });
  ...
  await page.getByText("Velero via Helm (cross-namespace check)").waitFor({
    state: "visible",
    timeout: 15_000,
  });
}

async function runHelmScenario(page) {
  console.log("[helm] open page");
  await page.goto(clusterRoute("helm"), {
    waitUntil: "domcontentloaded",
    timeout: 20_000,
  });
  await waitForPageToSettle(page, 4500);

  await page.getByRole("heading", { name: "Helm", exact: true }).waitFor({
    state: "visible",
    timeout: 15_000,
  });
  ...
  await page.getByText("Manage Helm repositories and releases for this cluster.").waitFor({
    state: "visible",
    timeout: 15_000,
  });
  const bodyText = await page.locator("body").innerText();
  assert.match(bodyText, /No Helm releases found\.|Repositories|Helm releases|Releases/i);
}
...
async function main() {
  ...
  try {
    await runDeprecationScanScenario(page);
    await runVersionAuditScenario(page);
    await runBackupAuditScenario(page);
    await runHelmScenario(page);
    await page.screenshot({
      path: "/tmp/live-qa-cluster-ops-smoke.png",
      fullPage: true,
    });
  } finally {
    await browser.close();
    ...
    JSON.stringify(
      {
        ...
        checkedWorkloads: ["deprecationscan", "versionaudit", "backupaudit", "helm"],
        screenshot: "/tmp/live-qa-cluster-ops-smoke.png",
      },
      null,
      2,
    ),
