<script lang="ts">
  ...
  import type { PVCVolume } from "$entities/pod";
  import * as Table from "$shared/ui/table";

  let expanded = false;

  export let volumes: PVCVolume[];
      ...
      <Table.Root>
        <Table.Header>
          <Table.Row>
            ...
            <Table.Head>Persistent Volume Claim</Table.Head>
          </Table.Row>
        </Table.Header>
        <Table.Body>
          {#each volumes as volume}
            <Table.Row>
              ...
              <Table.Cell>{volume.persistentVolumeClaim?.claimName}</Table.Cell>
            </Table.Row>
          {/each}
        </Table.Body>
      </Table.Root>
