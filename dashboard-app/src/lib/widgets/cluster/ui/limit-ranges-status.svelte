<script lang="ts">
  ...
  import { checkLimitRanges, updateClusterCheckPartially } from "$features/check-health";
  import type { LimitRangesReport } from "$features/check-health/model/types";

  interface Props {
    report: LimitRangesReport | null;
    clusterId: string;
  }
  ...
  const statusLabel = $derived.by(() => {
    ...
    if (summary.status === "ok")
      return `${summary.total} range${summary.total !== 1 ? "s" : ""}`;
  ...
  const detailText = $derived(
    summary?.message ?? "Limit ranges data is unavailable for this cluster.",
  );
  ...
  async function refreshNow() {
    ...
    isRefreshing = true;
    try {
      const fresh = await checkLimitRanges(clusterId, { force: true });
      await updateClusterCheckPartially(clusterId, { limitRanges: fresh });
    } finally {
      isRefreshing = false;
    }
  }
...
<div class="pl-1 flex justify-between items-center gap-2 mb-1 mt-1 text-sm">
  <span class="font-medium text-slate-700 dark:text-slate-300">Limit Ranges:</span>
    ...
    <Popover.Content class="w-[460px] max-h-[420px] overflow-y-auto" sideOffset={8}>
      ...
      <Table.Root>
        ...
        <Table.Body>
          ...
          {:else}
            <Table.Row>
              <Table.Cell colspan={2}>No limit ranges found in this cluster.</Table.Cell>
            </Table.Row>
          {/if}
        </Table.Body>
      </Table.Root>
