<script lang="ts">
  ...
  const trivyInstalled = $derived(
    providers.some((provider) => provider.id === "trivy-operator" && provider.status === "installed"),
  );
  ...
  let reportView = $state<"report" | "raw">("report");
  ...
  function reportViewText(): string {
    if (!reportRaw) return "";
    if (reportView === "raw") return reportRaw;
    ...
    const lines: string[] = [];
    lines.push("Trivy scan report");
    lines.push("");
    lines.push(`Generated: ${parsed.generatedAt ?? "-"}`);
    lines.push(
      `Summary: installed ${parsed.summary?.providersInstalled ?? 0} | findings ${parsed.summary?.findings ?? 0} | errors ${parsed.summary?.errors ?? 0}`,
    );
    const summarySeverity = parsed.summary?.severity ?? {};
    const severityParts = ["critical", "high", "medium", "low", "unknown"]
      .filter((key) => (summarySeverity[key] ?? 0) > 0)
      .map((key) => `${key} ${summarySeverity[key]}`);
    if (severityParts.length > 0) {
      lines.push(`Severity: ${severityParts.join(" | ")}`);
    }
    if ((parsed.summary?.checks?.total ?? 0) > 0) {
      lines.push(
        `Checks: failed ${parsed.summary?.checks?.failed ?? 0} | passed ${parsed.summary?.checks?.passed ?? 0} | total ${parsed.summary?.checks?.total ?? 0}`,
      );
    }
    lines.push("");
    ...
    for (const resource of parsed.resources ?? []) {
      ...
      const resourceSeverityParts = ["critical", "high", "medium", "low", "unknown"]
        .filter((key) => (resourceSeverity[key] ?? 0) > 0)
        .map((key) => `${key} ${resourceSeverity[key]}`);
      if (resourceSeverityParts.length > 0) {
        lines.push(`  severity: ${resourceSeverityParts.join(" | ")}`);
      }
      if ((resource.details?.checks?.total ?? 0) > 0) {
        lines.push(
          `  checks: failed ${resource.details?.checks?.failed ?? 0} | passed ${resource.details?.checks?.passed ?? 0} | total ${resource.details?.checks?.total ?? 0}`,
        );
      }
  ...
  async function handleInstall(providerId: TrivyProviderId, title: string) {
    ...
    actionNotice = null;
    try {
      const result = await installTrivyProvider(activeClusterId, providerId);
  ...
  <Card.Header class="flex flex-col gap-2">
    <div class="flex flex-wrap items-center justify-between gap-2">
      <div class="flex items-center gap-2">
        <h2 class="text-lg font-semibold" title="Trivy Operator status and latest vulnerability scan snapshot.">
          Trivy
        </h2>
        {#if summary}
        ...
        <Popover.Root>
          ...
          <Popover.Content class="w-[420px] space-y-3" sideOffset={8}>
            <p class="text-sm font-semibold text-foreground">Trivy Operator integration</p>
            <p class="text-xs text-muted-foreground">
              Trivy Operator scans cluster workloads and produces security reports via Kubernetes CRDs.
            </p>
            <a
    ...
    <p class="text-sm text-muted-foreground">
      Manual mode: Refresh status updates provider status only. Run Trivy scan creates a new report.
    </p>
    {#if !trivyInstalled}
      <p class="text-xs text-amber-600">
        Install Trivy Operator first to enable scanning.
      </p>
    {/if}
  </Card.Header>
  <Card.Content class="space-y-6">
    ...
    <div class="grid gap-4 md:grid-cols-2">
      ...
      <DiagnosticSummaryCard title="Summary">
        ...
        <p class="text-xs text-muted-foreground">
          Providers detected: {providers.filter((p) => p.status === "installed").length}/{providers.length}
        </p>
      </DiagnosticSummaryCard>
    </div>
    ...
    {#if reportRaw}
      <div class="rounded-lg border border-border bg-muted/20 p-3">
        <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
          <p class="text-xs font-semibold text-foreground">Latest Trivy scan report</p>
          {#if hasJsonReport()}
            <div class="flex items-center gap-2">
              <select
