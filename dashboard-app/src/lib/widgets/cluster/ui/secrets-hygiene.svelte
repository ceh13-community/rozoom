<script lang="ts">
  ...
  import { checkSecretsHygiene, updateClusterCheckPartially } from "$features/check-health";
  import type { SecretsHygieneReport } from "$features/check-health/model/types";

  interface Props {
    report: SecretsHygieneReport | null;
    clusterId: string;
  }
  ...
  const detailText = $derived(
    summary?.message ? `Secrets: ${summary.message}` : "Secrets: Unknown",
  );
  ...
  async function refreshNow() {
    ...
    isRefreshing = true;
    try {
      const fresh = await checkSecretsHygiene(clusterId, { force: true });
      await updateClusterCheckPartially(clusterId, { secretsHygiene: fresh });
    } finally {
      isRefreshing = false;
    }
  }
...
<div class="pl-1 flex justify-between items-center gap-2 mb-1 mt-1 text-sm">
  <span class="font-medium text-slate-700 dark:text-slate-300">Secrets hygiene:</span>
  <Popover.Root bind:open={isOpen}>
    ...
    <Popover.Content class="w-[980px] max-h-[520px] overflow-y-auto" sideOffset={8}>
      ...
      <div class="mb-3 text-xs text-muted-foreground">
        Encryption at rest: {report?.encryptionStatus ?? "unknown"}
      </div>
      ...
      <Table.Root>
        <Table.Header>
          <Table.Row>
            ...
            <Table.Head>ConfigMap</Table.Head>
        ...
        <Table.Body>
          {#if items.length}
            {#each items as item (item.namespace + item.configMap + item.key)}
              <Table.Row>
          ...
          {:else}
            <Table.Row>
              <Table.Cell colspan={4}>No suspicious ConfigMap entries detected.</Table.Cell>
            </Table.Row>
          {/if}
        </Table.Body>
      </Table.Root>
