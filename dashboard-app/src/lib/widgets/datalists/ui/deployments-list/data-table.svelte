<script
  lang="ts"
  generics="TData extends { uid: string; name: string; namespace: string; node: string }"
>
  import {
    type ColumnDef,
    type SortingState,
  ...
  import { ChevronDown, Layers3, List, Server } from "$shared/ui/icons";
  import DeploymentSelectionCheckbox from "./deployment-selection-checkbox.svelte";
  ...
  type DataTableProps<TData> = {
    ...
    onViewModeChange: (mode: DeploymentsTableViewMode) => void;
  ...
  function getGroupedRows() {
    ...
    for (const row of rows) {
      const key =
        viewMode === "node" ? row.original.node?.trim() || "Unscheduled" : row.original.namespace;
  ...
  async function downloadCsv() {
    ...
    const result = await exportCsvArtifact({ filename: "deployments.csv", csv });
...
<TableSurface onScroll={handleTableScroll}>
  <Table.Root>
    <Table.Caption>A list of deployments.</Table.Caption>
    <Table.Header>
            ...
            <Table.Cell colspan={columns.length} class="sticky-table-group-header">
              <div class="inline-flex items-center gap-2">
                ...
                >
                  ...
                  />
                  <span>{viewMode === "node" ? "Node" : "Namespace"}: {groupKey || "-"}</span>
