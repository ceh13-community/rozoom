export type StatefulSetsWorkbenchLayout = "single" | "dual" | "triple";

export type PersistedStatefulSetsWorkbenchTab = {
...
export type PersistedStatefulSetsClosedTab = {
  kind: "logs" | "yaml" | "events" | "rollout-status" | "rollout-history";
  target: { name: string; namespace: string } | null;
  pinned: boolean;
};

export type PersistedStatefulSetsWorkbenchState = {
  version: 1;
  tabs: PersistedStatefulSetsWorkbenchTab[];
  activeTabId: string | null;
  layout: StatefulSetsWorkbenchLayout;
  paneTabIds: [string | null, string | null, string | null];
  collapsedPaneIndexes: number[];
  closedTabs: PersistedStatefulSetsClosedTab[];
  workbenchCollapsed: boolean;
  workbenchFullscreen: boolean;
};

const KEY_PREFIX = "dashboard.statefulsets.workbench.state.v1";

export function getStatefulSetsWorkbenchStateKey(clusterId: string) {
...
export function parseStatefulSetsWorkbenchState(
  raw: string | null,
): PersistedStatefulSetsWorkbenchState | null {
  if (!raw) return null;
  try {
    ...
    const tabs = parsed.tabs
      ...
      .flatMap((tab) => {
        ...
        return [
          ...
          } satisfies PersistedStatefulSetsWorkbenchTab,
        ];
      });
      ...
      .flatMap((entry) => {
        ...
        const target =
          targetRaw && typeof targetRaw.name === "string" && typeof targetRaw.namespace === "string"
            ...
            : null;
        return [
          { kind, target, pinned: Boolean(entry.pinned) } satisfies PersistedStatefulSetsClosedTab,
        ];
      });

    const layoutRaw = parsed.layout;
    const layout: StatefulSetsWorkbenchLayout =
