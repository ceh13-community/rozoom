export type JobsWorkbenchLayout = "single" | "dual" | "triple";

export type PersistedJobsWorkbenchTab = {
  kind: "logs" | "yaml";
  name: string;
  namespace: string;
  pinned: boolean;
...
export type PersistedJobsClosedTab = {
  kind: "logs" | "yaml";
  target: { name: string; namespace: string } | null;
  pinned: boolean;
};

export type PersistedJobsWorkbenchState = {
  version: 1;
  tabs: PersistedJobsWorkbenchTab[];
  activeTabId: string | null;
  layout: JobsWorkbenchLayout;
  paneTabIds: [string | null, string | null, string | null];
  collapsedPaneIndexes: number[];
  closedTabs: PersistedJobsClosedTab[];
  workbenchCollapsed: boolean;
  workbenchFullscreen: boolean;
};

const KEY_PREFIX = "dashboard.jobs.workbench.state.v1";

export function getJobsWorkbenchStateKey(clusterId: string) {
...
export function parseJobsWorkbenchState(raw: string | null): PersistedJobsWorkbenchState | null {
  if (!raw) return null;
  try {
    ...
    const tabs = parsed.tabs
      ...
      .flatMap((tab) => {
        ...
        return [
          ...
          } satisfies PersistedJobsWorkbenchTab,
        ];
      });
      ...
      .flatMap((entry) => {
        ...
        const target =
          targetRaw && typeof targetRaw.name === "string" && typeof targetRaw.namespace === "string"
            ...
            : null;
        return [{ kind, target, pinned: Boolean(entry.pinned) } satisfies PersistedJobsClosedTab];
      });

    const layoutRaw = parsed.layout;
    const layout: JobsWorkbenchLayout =
