<script lang="ts" generics="TData extends { uid: string; name: string; namespace: string }">
  ...
  export type JobsTableViewMode = "flat" | "namespace";
  ...
  type DataTableProps<TData> = {
    ...
    onViewModeChange: (mode: JobsTableViewMode) => void;
  ...
  async function downloadCsv() {
    ...
    const result = await exportCsvArtifact({ filename: "jobs.csv", csv });
...
<TableSurface onScroll={handleTableScroll}>
  <Table.Root>
    <Table.Caption>A list of jobs.</Table.Caption>
    <Table.Header>
