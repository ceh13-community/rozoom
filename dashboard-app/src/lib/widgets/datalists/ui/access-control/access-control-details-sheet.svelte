<script lang="ts">
  ...
  import type { AccessControlRow } from "./model/access-control-row-adapter";
  ...
  interface Props {
    row: AccessControlRow | null;
    isOpen: boolean;
    onClose: () => void;
    onOpenYaml: (row: AccessControlRow) => void;
    onCopyKubectlGetYaml: (row: AccessControlRow) => void;
    onCopyKubectlDescribe: (row: AccessControlRow) => void;
    onRunDebugDescribe: (row: AccessControlRow) => void;
    onInvestigate?: (row: AccessControlRow) => void;
  ...
  function getFields() {
    ...
    return [
      ...
      { label: "Subjects", value: row.subjects },
      { label: "Rules", value: row.rules },
...
{#if isOpen && row}
<DetailsSheetPortal open={isOpen} onClose={onClose} closeAriaLabel="Close access-control details" maxWidthClass="sm:max-w-[44rem]">
      <DetailsSheetHeader
        title="Access-control resource"
        name={row.name}
        icon={Info}
        onClose={onClose}
        closeAriaLabel="Close access-control details"
        actions={[
          {
            id: "edit-yaml",
            title: "Edit YAML",
            ariaLabel: "Edit access-control YAML",
            icon: Pencil,
            ...
            ? [
                {
                  id: "investigate",
                  title: "Investigate",
                  ariaLabel: "Investigate access-control resource",
                  icon: Search,
            ...
            : []),
          {
            id: "delete",
            title: "Delete",
            ariaLabel: "Delete access-control resource",
            icon: Trash,
            destructive: true,
      ...
      <div class="flex-1 space-y-4 overflow-auto px-4 py-4">
        <DetailsMetadataGrid
          ...
          contextKey={`access-control-details:${row.uid}`}
        />
      </div>
</DetailsSheetPortal>
{/if}
