<script lang="ts" generics="TData extends { uid: string; name: string; namespace: string }">
  ...
  export type CronJobsTableViewMode = "flat" | "namespace";
  ...
  type DataTableProps<TData> = {
    ...
    onViewModeChange: (mode: CronJobsTableViewMode) => void;
  ...
  async function downloadCsv() {
    ...
    const result = await exportCsvArtifact({ filename: "cron-jobs.csv", csv });
...
<TableSurface onScroll={handleTableScroll}>
  <Table.Root>
    <Table.Caption>A list of cron jobs.</Table.Caption>
    <Table.Header>
