<script lang="ts" generics="TData extends { uid: string; name: string; namespace: string }">
  ...
  export type DaemonSetsTableViewMode = "flat" | "namespace";
  ...
  type DataTableProps<TData> = {
    ...
    onViewModeChange: (mode: DaemonSetsTableViewMode) => void;
  ...
  async function downloadCsv() {
    ...
    const result = await exportCsvArtifact({ filename: "daemon-sets.csv", csv });
...
<TableToolbarShell>
  {#snippet children()}
    <Input
      placeholder="Filter daemon sets..."
...
<TableSurface onScroll={handleTableScroll}>
  <Table.Root>
    <Table.Caption>A list of daemon sets.</Table.Caption>
    <Table.Header>
    ...
    <Table.Body>
      {#if viewMode === "flat"}
        ...
        {/if}
      {:else}
        {#each getGroupedRows() as [groupKey, groupRows] (getGroupId(groupKey))}
          <Table.Row class="bg-muted/40">
            <Table.Cell colspan={columns.length} class="sticky-table-group-header">
              <div class="inline-flex items-center gap-2">
                <DaemonSetSelectionCheckbox
                  checked={getGroupSelectionState(groupRows).checked}
                  indeterminate={getGroupSelectionState(groupRows).indeterminate}
                  label={`Select namespace ${groupKey || "-"}`}
                  onToggle={(next) =>
                    onToggleGroupSelection(
                      groupKey,
                      next,
                      groupRows.map((row) => row.original.uid),
                    )}
                />
                <button
                  type="button"
                ...
                >
                  ...
                  <span class="opacity-80">({groupRows.length})</span>
                </button>
              </div>
            </Table.Cell>
          </Table.Row>
