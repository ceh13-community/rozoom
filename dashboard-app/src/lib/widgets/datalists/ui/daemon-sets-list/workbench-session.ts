export type DaemonSetsWorkbenchLayout = "single" | "dual" | "triple";

export type PersistedDaemonSetsWorkbenchTab = {
...
export type PersistedDaemonSetsClosedTab = {
  kind: "logs" | "yaml" | "events" | "rollout-status" | "rollout-history";
  target: { name: string; namespace: string } | null;
  pinned: boolean;
};

export type PersistedDaemonSetsWorkbenchState = {
  version: 1;
  tabs: PersistedDaemonSetsWorkbenchTab[];
  activeTabId: string | null;
  layout: DaemonSetsWorkbenchLayout;
  paneTabIds: [string | null, string | null, string | null];
  collapsedPaneIndexes: number[];
  closedTabs: PersistedDaemonSetsClosedTab[];
  workbenchCollapsed: boolean;
  workbenchFullscreen: boolean;
};

const KEY_PREFIX = "dashboard.daemonsets.workbench.state.v1";

export function getDaemonSetsWorkbenchStateKey(clusterId: string) {
...
export function parseDaemonSetsWorkbenchState(
  raw: string | null,
): PersistedDaemonSetsWorkbenchState | null {
  if (!raw) return null;
  try {
      ...
      .flatMap((entry) => {
        ...
        const target =
          targetRaw && typeof targetRaw.name === "string" && typeof targetRaw.namespace === "string"
            ...
            : null;
        return [
          { kind, target, pinned: Boolean(entry.pinned) } satisfies PersistedDaemonSetsClosedTab,
        ];
      });

    const layoutRaw = parsed.layout;
    const layout: DaemonSetsWorkbenchLayout =
