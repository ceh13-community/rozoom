export type ReplicaSetsWorkbenchLayout = "single" | "dual" | "triple";

export type PersistedReplicaSetsWorkbenchTab = {
  kind: "logs" | "yaml";
  name: string;
  namespace: string;
  pinned: boolean;
...
export type PersistedReplicaSetsClosedTab = {
  kind: "logs" | "yaml";
  target: { name: string; namespace: string } | null;
  pinned: boolean;
};

export type PersistedReplicaSetsWorkbenchState = {
  version: 1;
  tabs: PersistedReplicaSetsWorkbenchTab[];
  activeTabId: string | null;
  layout: ReplicaSetsWorkbenchLayout;
  paneTabIds: [string | null, string | null, string | null];
  collapsedPaneIndexes: number[];
  closedTabs: PersistedReplicaSetsClosedTab[];
  workbenchCollapsed: boolean;
  workbenchFullscreen: boolean;
};

const KEY_PREFIX = "dashboard.replicasets.workbench.state.v1";

export function getReplicaSetsWorkbenchStateKey(clusterId: string) {
...
export function parseReplicaSetsWorkbenchState(
  raw: string | null,
): PersistedReplicaSetsWorkbenchState | null {
  if (!raw) return null;
  try {
    ...
    const tabs = parsed.tabs
      ...
      .flatMap((tab) => {
        ...
        return [
          ...
          } satisfies PersistedReplicaSetsWorkbenchTab,
        ];
      });
      ...
      .flatMap((entry) => {
        ...
        const target =
          targetRaw && typeof targetRaw.name === "string" && typeof targetRaw.namespace === "string"
            ...
            : null;
        return [
          { kind, target, pinned: Boolean(entry.pinned) } satisfies PersistedReplicaSetsClosedTab,
        ];
      });

    const layoutRaw = parsed.layout;
    const layout: ReplicaSetsWorkbenchLayout =
