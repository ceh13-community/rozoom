<script lang="ts" generics="TData extends { uid: string; name: string; namespace: string }">
  ...
  export type ReplicaSetsTableViewMode = "flat" | "namespace";
  ...
  type DataTableProps<TData> = {
    ...
    onViewModeChange: (mode: ReplicaSetsTableViewMode) => void;
  ...
  async function downloadCsv() {
    ...
    const result = await exportCsvArtifact({ filename: "replica-sets.csv", csv });
...
<TableToolbarShell>
  {#snippet children()}
    <Input
      placeholder="Filter replica sets..."
...
<TableSurface onScroll={handleTableScroll}>
  <Table.Root>
    <Table.Caption>A list of replica sets.</Table.Caption>
    <Table.Header>
