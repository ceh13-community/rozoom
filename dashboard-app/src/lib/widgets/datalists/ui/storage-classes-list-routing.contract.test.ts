...
describe("storage classes list routing contract", () => {
  it("routes storage classes through the shared workload-reference boundary", () => {
    expect(workloadDisplaySource).toContain(
      'storageclasses: () => import("$widgets/datalists/ui/configuration-list.svelte")',
    );
  });

  it("keeps storage classes inside the shared workload shell", () => {
    ...
    expect(configurationListSource).toContain("storageclasses");
  });
});
