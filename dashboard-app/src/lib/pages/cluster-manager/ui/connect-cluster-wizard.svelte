<script lang="ts">
  ...
  async function loadScopesForProvider(provider: string = cloudProvider) {
    ...
    // Discard stale response if user switched provider mid-request.
...
>
  ...
  >
    <div>
      ...
      <p class="text-xs text-slate-400 mt-0.5">
        OIDC / SSO, Cloud Provider, Vault, Certificate, Token
      </p>
    </div>
  ...
  <div class="px-6 pb-6 pt-2">
    {#if success}
      <div
        class="text-xs text-emerald-500 bg-emerald-500/10 border border-emerald-500/30 rounded px-3 py-2 mb-3 flex justify-between items-center"
      >
        <span>{success}</span>
        <button
    ...
    {#if error}
      <div
        class="text-xs text-rose-500 bg-rose-500/10 border border-rose-500/30 rounded px-3 py-2 mb-3 flex justify-between items-center"
      >
        <span>{error}</span>
    ...
    {#if !method}
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
        {#each [{ id: "oidc", label: "OIDC / SSO", desc: "Azure AD, Okta, Keycloak, Google", icon: "🔐", security: "high" }, { id: "cloud", label: "Cloud Provider", desc: "AWS EKS, GKE, AKS (one-click)", icon: "☁️", security: "high" }, { id: "vault", label: "HashiCorp Vault", desc: "Dynamic credentials from Vault", icon: "🏦", security: "high" }, { id: "certificate", label: "X.509 Certificate", desc: "Client cert + key (mTLS)", icon: "📜", security: "high" }, { id: "token", label: "Bearer Token", desc: "Static or service account token", icon: "🎫", security: "medium" }] as option (option.id)}
          <button
            ...
            onclick={() => {
              method = option.id as ConnectMethod;
              clearMessages();
              if (option.id === "cloud") void loadScopesForProvider();
            }}
          >
            ...
            <span
              class="text-[9px] mt-1 inline-block px-1.5 py-0.5 rounded-full {option.security ===
              'high'
